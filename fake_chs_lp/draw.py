...
class Draw:
    def __init__(self, bg):
        ...
        if bg == "black":
            ...
            self._bg = cv2.resize(cv2.imread(r"E:\python\pyProject\deepLearning\LicensePlate-master\fake_chs_lp\res\black_bg.png"), self.size)
        elif bg == "blue":
            self.color = [(0, 0, 0), (255, 255, 255)]
            self._bg = cv2.resize(cv2.imread(r"E:\python\pyProject\deepLearning\LicensePlate-master\fake_chs_lp\res\blue_bg.png"), self.size)
        elif bg == "green_0":
            self.city = 43
            self.size = (480, 140)
            self.plane = (25, 115, 43, 9, 49)
            self.color = [(255, 255, 255), (0, 0, 0)]
            self._bg = cv2.resize(cv2.imread(r"E:\python\pyProject\deepLearning\LicensePlate-master\fake_chs_lp\res\green_bg_0.png"), self.size)
        elif bg == "green_1":
            self.city = 43
            self.size = (480, 140)
            self.plane = (25, 115, 43, 9, 49)
            self.color = [(255, 255, 255), (0, 0, 0)]
            self._bg = cv2.resize(cv2.imread(r"E:\python\pyProject\deepLearning\LicensePlate-master\fake_chs_lp\res\green_bg_1.png"), self.size)
        elif bg == "yellow":
            self.color = [(255, 255, 255), (0, 0, 0)]
            self._bg = cv2.resize(cv2.imread(r"E:\python\pyProject\deepLearning\LicensePlate-master\fake_chs_lp\res\yellow_bg.png"), self.size)
